...
#
# build_tools - Build single-file tools from design documents
#
# This workflow reads a design created by design_program and generates
# complete working code with tests and documentation for standalone tools.
...
export WORKFLOW_NAME="build_tools"
...
ask_clarifying_questions() {
    ...
    QUESTIONS=$(ci <<EOF
You are about to build this tool:
...
build_tool() {
    ...
    # Step 1: Generate main tool file
    step "1/4" "Generating source code..."

    PROGRAM_CODE=$(ci <<EOF
Build the complete tool based on this design:
...
Generate the complete, working source code for the main tool file in $LANGUAGE.
...
EOF
    ...
    TEST_CODE=$(ci <<EOF
Generate a comprehensive test suite for this tool:
...
offer_installation() {
    banner_summary "Tool Built Successfully!"

    echo -e "${BOLD}${WHITE}Tool:${NC} $(path $PROGRAM_NAME)"
...
# Main execution
main() {
    banner_welcome "Welcome to Tool Builder!" \
                   "AI-powered single-file tool generation from design documents"
    ...
    # Test it
    test_tool || warning "Continue with manual testing"
