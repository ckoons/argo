...
static size_t write_callback(void* contents, size_t size, size_t nmemb, void* userp) {
    ...
    ci_http_response_t* resp = (ci_http_response_t*)userp;
...
const char* ci_get_daemon_url(void) {
    ...
    if (!initialized) {
        int port = CI_DAEMON_DEFAULT_PORT;
...
int ci_http_post(const char* endpoint, const char* json_body, ci_http_response_t** response) {
    ...
    int result = ci_ensure_daemon_running();
    ...
    curl_easy_setopt(curl, CURLOPT_TIMEOUT, 120L);  /* Longer timeout for AI queries */
