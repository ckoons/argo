...
ci_provider_t* ollama_create_provider(const char* model_name) {
    ...
    if (!ctx) {
        LOG_ERROR("Failed to allocate Ollama context");
        return NULL;
    }
...
static int ollama_connect(ci_provider_t* provider) {
    ...
    if (ctx->socket_fd < 0) {
        LOG_ERROR("Failed to connect to Ollama at localhost:%d", ctx->port);
...
static int ollama_query(ci_provider_t* provider, const char* prompt,
    ...
    /* Build HTTP request */
    char request[8192];
    int req_len = build_http_request(request, sizeof(request), ctx->model, prompt);
    if (req_len < 0) {
    ...
    if (sent != req_len) {
        LOG_ERROR("Failed to send request to Ollama: %s", strerror(errno));
    ...
    if (!json_start) {
        LOG_ERROR("No JSON response from Ollama");
        ollama_disconnect(ctx);
    ...
    if (!response_start) {
        LOG_ERROR("No 'response' field in JSON");
        ollama_disconnect(ctx);
        return E_PROTOCOL_FORMAT;
    }
    ...
    if (*p != '"') {
        LOG_ERROR("Unterminated response string");
        ollama_disconnect(ctx);
        return E_PROTOCOL_FORMAT;
    }
...
static int ollama_stream(ci_provider_t* provider, const char* prompt,
    ...
    /* Similar to query but calls callback for each chunk */
    /* Implementation would be similar to ollama_query but with per-chunk callbacks */
    return E_INTERNAL_NOTIMPL;  /* TODO: Implement if needed */
}
...
/* Note: Streaming parsing removed - using non-streaming API for now */
