...
int registry_send_message(ci_registry_t* registry,
    ...
    LOG_DEBUG("Message from %s to %s: %s", from_ci, to_ci, message_json);
    return ARGO_SUCCESS;
}

/* Broadcast message (stub) */
int registry_broadcast_message(ci_registry_t* registry,
    ...
    LOG_DEBUG("Broadcast from %s to role %s: %s",
             from_ci, role_filter ? role_filter : "all", message_json);
    return ARGO_SUCCESS;
}
