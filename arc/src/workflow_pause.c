...
int arc_workflow_pause(int argc, char** argv) {
    ...
    if (argc >= 1) {
        ...
        if (!workflow_name) {
            ...
            fprintf(stderr, "Usage: arc workflow pause [workflow_name]\n");
            fprintf(stderr, "   or: arc switch [workflow_name] && arc workflow pause\n");
    ...
    if (wf->status == WORKFLOW_STATUS_SUSPENDED) {
        fprintf(stderr, "Workflow already paused: %s\n", workflow_name);
        workflow_registry_destroy(registry);
        argo_exit();
